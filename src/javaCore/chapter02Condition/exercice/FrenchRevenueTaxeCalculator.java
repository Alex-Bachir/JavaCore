...
public class FrenchRevenueTaxeCalculator {
    ...
    // ici on applique l'abatement de 10%
    public static double applyTaxAllowance(double salairNet, double taxAllowancePercentage) {

        double netSalaryAfterAbattement;
        double abbatementMaxAmount = 10000;
        double abbatement = salairNet * taxAllowancePercentage;

        if (abbatement >= abbatementMaxAmount) {
            netSalaryAfterAbattement = salairNet - abbatementMaxAmount;
        } else {
            netSalaryAfterAbattement = salairNet - abbatement;
        }

        return netSalaryAfterAbattement;
    }

    // ici on applique la formule pour les taxes en fonction du salaire
    public static double applyRevenueTax(double netSalaryAfterAbattement) {
        ...
        if (netSalaryAfterAbattement > TAX_BRACKET_5) {
            taxBracket5 = ((netSalaryAfterAbattement - TAX_BRACKET_5) * TAX_RATE_5);
            netSalaryAfterAbattement = TAX_BRACKET_5;
        }

        if (netSalaryAfterAbattement >= TAX_BRACKET_4) {
            taxBracket4 = ((netSalaryAfterAbattement - TAX_BRACKET_4) * TAX_RATE_4);
            netSalaryAfterAbattement = TAX_BRACKET_4;
        }

        if (netSalaryAfterAbattement >= TAX_BRACKET_3) {
            taxBracket3 = ((netSalaryAfterAbattement - TAX_BRACKET_3) * TAX_RATE_3);
            netSalaryAfterAbattement = TAX_BRACKET_3;
        }

        if (netSalaryAfterAbattement >= TAX_BRACKET_2) {
            taxBracket2 = ((netSalaryAfterAbattement - TAX_BRACKET_2) * TAX_RATE_2);
            netSalaryAfterAbattement = TAX_BRACKET_2;
        }

        if (netSalaryAfterAbattement < TAX_BRACKET_2) {
            taxBracket1 = ((netSalaryAfterAbattement - TAX_BRACKET_2) * TAX_RATE_1);
            netSalaryAfterAbattement = TAX_BRACKET_2;
        }

        taxTotal = taxBracket5 + taxBracket4 + taxBracket3 + taxBracket2 + taxBracket1;
